...
**Интеллектуальная система анализа рекламных кампаний в Binom (v1) с AI-модулями**
...
- Синхронизация с Binom API (v1) - кампании, источники, офферы, партнерки
- Запуск аналитики по расписанию, уведомления в telegram

### 42 настраиваемых модуля аналитики
Модули разделены на 9 категорий:

- **Критические алерты (6)**: Детекторы утечки бюджета, нулевых апрувов, скачков расхода
- **Анализ трендов (5)**: Сканеры микротрендов, разворотов, восстановлений
- **Стабильность (4)**: Оценка стабильности, волатильности, надежности
- **Предиктивная аналитика (5)**: Прогнозы ROI, апрувов, жизненного цикла кампаний
- **Детекция проблем (6)**: Спящие кампании, проблемы с маржой, зомби-кампании
- **Поиск возможностей (4)**: Скрытые точки роста, кандидаты на масштабирование
- **Группировка (3)**: Умная группировка и консолидация кампаний
- **Портфельная аналитика (5)**: Оценка здоровья портфеля, диверсификации, оптимизация бюджета
- **Источники и офферы (4)**: Рейтинги офферов, качество источников, мониторинг сетей
...
**Подготовьте ваш Binom (v1) URL и API key (Telegram и OpenRouter - опционально)**
...
**Протестирована только установка "IP:port" на VPS с Ubuntu** 
